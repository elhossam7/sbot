...
// Buy handler
export const handleBuyCommand = async (ctx) => {
...
export async function handleSellCommand(ctx) {
    try {
        const result = await executeSellOrder(new Connection(config.RPC_URL), ctx.tokenMint, ctx.amount, ctx.minPrice);
...
function parseTradeParams(message) {
    if (!message)
        return null;
